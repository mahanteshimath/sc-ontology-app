...
/**
 * NOTE ON WINDOWS: the fixture keys below use POSIX separators, because that is what these paths
 * look like in SPCS, where the code actually runs. lib/snowflake.ts builds them with path.join,
 * which emits backslashes on Windows, so six of the getSecret mounted-file assertions in this file
 * fail when the suite is run on Windows and pass on Linux/macOS and in CI. That is a property of
 * the fixtures, not of the code under test. Normalising separators here is not the fix: it makes
 * the TOML fixtures start matching paths they were never meant to match, which turns six
 * platform-specific failures into fifteen real ones. Port the fixtures to path.join if this ever
 * needs to pass on Windows.
 */
